...
function getUsersFromDatabase(array) {
  ...
  return Array.from(array, (users) => {
    return {
      id: users.id,
      firstName: users.name.toUpperCase(),
      years: users.age,
    };
  });
}
...
function combineAndReverseArrays(arr1, arr2) {
  ...
  if (!Array.isArray(arr1) && !Array.isArray(arr2)) {
    ...
    let reverseArr = combineArr.reverse();
    return reverseArr;
  }
}
...
function customUnshift(arr, ...elements) {
  ...
  for (let i = elements.length - 1; i >= 0; i--) {
    let newLength = initialLength + elements.length;
...
function customAt(arr, index) {
  ...
  let getElem = arr.at(index);

  if (typeof getElem === "string") {
    console.log("Елемент є рядком");
  } else if (typeof getElem === "number") {
    console.log("Елемент є числом");
  } else if (typeof getElem === "object") {
